...
`pnpm --filter @deepseek-ai/dsh-enterprise-desktop build:provider` 构建 [Cordis 插件](src/gateway-provider.ts)，导出路径为 `@deepseek-ai/dsh-enterprise-desktop/gateway`。它在 `ctx.llm` 注册 `enterprise` 路由。可信 profile 提供 API origin、Keychain 辅助程序及账号定位信息、传输宽限超时和事件／响应限制，不提供模型提供方密钥。适配器验证凭据与部署／设备的绑定，续期 Runtime 租约，并在发送调用前读取平台启用的模型 ID。模型调用先使用目录中的超时，再等待传输宽限超时，因此 API 拥有上游截止时间，并能在桌面取消前返回失败。API 验证 Runtime 并解析平台模型后转发调用。

[请求与流转换](src/gateway-wire.ts) 按所选目录条目把 DSH 消息映射为 OpenAI Chat Completions 或 Responses。它通过附件服务解析持久图片、视频、音频和文档；提供方文件策略使用共享 `llm-files` 协调器，有界内联策略则生成请求范围内的 Base64。目录元数据会另外说明原生视频只覆盖画面，还是也理解内嵌音轨；独立音频输入仍使用 `audio` 模态，两种视频模式则使用相同的提供方视频协议项。Responses 历史把 assistant 文本生成为已完成的原生消息项，并保留它与 reasoning 及后续媒体输入的顺序。Responses 工具结果把文本保留为有关联关系的 `function_call_output` 字符串，并把媒体放入相邻的用户输入项，因为 Ark 会拒绝结构化的 `function_call_output.output` 值。企业 API 校验模态声明与限制，使用服务端凭据上传提供方文件，并在不转换协议的情况下转发所选响应。适配器接受初始事件省略 `arguments` 的 Responses 函数调用项，累积后续参数增量，并要求完成项包含最终字符串。它把 reasoning-summary 记录投影为 DSH reasoning 块，并把每个非空的加密 reasoning 项与对应的原生函数调用一起存入带版本且按块对齐的适配器回放状态。后续工具轮次会在 `function_call_output` 之前按原顺序恢复这些原生项；初始事件中的空 `encrypted_content` 会被忽略，已经出现但不是字符串的协议字段则会关闭式失败。回放状态不包含附件数据或提供方文件标识符。适配器把 HTTP 429 分类为 `RATE_LIMIT`，把网关超时分类为 `TIMEOUT`，严格拒绝未知流式事件，并在取消时关闭附件读取、上传和模型响应。
