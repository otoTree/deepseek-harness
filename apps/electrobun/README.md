...
The [request and stream projection](src/gateway-wire.ts) maps DSH messages to the default OpenAI chat request and asks streaming providers to include their token usage. The enterprise API then forwards the requested path, method, body fields, headers, and response bytes without injecting or clamping OpenAI options; it replaces only the configured upstream model ID and Authorization header. Auxiliary compaction and Session-title purposes reach the same gateway. The adapter validates the streamed chat response for DSH consumption; the API observes compatible usage records for platform metering without rebuilding the response. Cancellation closes the owned response.
