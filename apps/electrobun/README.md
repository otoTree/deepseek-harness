...
`pnpm --filter @deepseek-ai/dsh-enterprise-desktop build:provider` builds the [Cordis plugin](src/gateway-provider.ts), exported as `@deepseek-ai/dsh-enterprise-desktop/gateway`. It registers the `enterprise` route on `ctx.llm`. The trusted profile supplies the API origin, Keychain helper/account locator, transport grace timeout, and event/response limits; it never supplies a provider key. The adapter verifies the credential's deployment/device binding, renews the Runtime lease, and retrieves enabled platform model IDs before dispatch. A model call uses the catalog timeout followed by the transport grace timeout, so the API owns the upstream deadline and can return its failure before the desktop cancels. The API authenticates the runtime and resolves the platform model before forwarding the call.

The [request and stream projection](src/gateway-wire.ts) maps DSH messages to either OpenAI Chat Completions or Responses according to the selected catalog entry. It resolves durable images, video, audio, and documents through the attachment service; provider-file policies use the shared `llm-files` coordinator, while bounded inline policies emit request-local Base64. Catalog metadata separately states whether native video covers frames only or also its embedded audio track; standalone audio input remains the `audio` modality, and both video modes use the provider's same video wire item. Responses history emits assistant text as a completed native message item, preserving its order with reasoning and later media input. Responses tool results preserve the text as a correlated `function_call_output` string and place media in an adjacent user input item because Ark rejects structured `function_call_output.output` values. The enterprise API validates modality claims and limits, uploads provider files with server credentials, and forwards the selected protocol response without converting it. The adapter accepts a Responses function-call item whose initial event omits `arguments`, accumulates subsequent argument deltas, and requires the completed item to contain the final string. It projects reasoning-summary records as DSH reasoning blocks and stores each non-empty encrypted reasoning item with the corresponding native function call in versioned, block-aligned adapter replay state. A later tool round restores those native items in their original order before `function_call_output`; preliminary empty `encrypted_content` values are ignored, while present non-string protocol fields fail closed. The replay state contains neither attachment data nor provider file identifiers. The adapter classifies HTTP 429 as `RATE_LIMIT` and gateway timeouts as `TIMEOUT`, strictly rejects unknown streaming events, and closes attachment reads, uploads, and the model response on cancellation.
