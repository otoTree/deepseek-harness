...
The [console](app/console.tsx) includes management login, organization units, invitations, member status, models, devices, session reads, usage, audit, plugin review, and platform settings. It does not expose user registration or desktop consent.
...
- The console does not expose every API operation: scoped roles, unit move/delete, invitation revocation, all-member session listing, and plugin review need completed UI flows.
