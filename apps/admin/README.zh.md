...
[控制台](app/console.tsx) 包含管理登录、根节点优先的全局组织树、跨组织账号详情、全平台模型目录、组织单元、邀请、成员状态、设备、会话读取、用量、审计、插件审核和平台设置。用量看板默认展示最近 30 个 Asia/Shanghai 自然日，并根据已结算的人民币价格快照提供平台汇总、每日趋势、模型／账号／组织／用途分组和分页调用明细。账号筛选会搜索平台目录，不会把选择范围限制在第一页。模型配置以人民币元／百万 token 录入普通输入、缓存输入和输出价格。启用的模型对所有组织可用；控制台不提供按组织分配模型。它不提供用户注册或桌面授权。
