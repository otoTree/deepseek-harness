...
The host is the authoritative attachment preflight point. It validates the complete batch against current byte, count, aggregate, media, dimension, and pixel policy before writing an attachment or event; its rejection appears through the composer's transient toast. Native model capability does not gate admission: an image-capable route receives the durable image block, while a route that explicitly excludes image input receives deterministic text with the current tool-readable normalized path when available ([projection decision](../architecture/2026-09-17-tool-readable-images-and-video-audio-capability.md)). This lets a tool that returns text inspect the saved attachment; without such a tool or path, the model must report the limitation. `session.updateQueue` edits accept text content only, so a queue edit cannot inject an image past attachment admission. The browser rejects unsupported declared image media types before allocating preview URLs, but it does not snapshot deployment limits or model capability.
