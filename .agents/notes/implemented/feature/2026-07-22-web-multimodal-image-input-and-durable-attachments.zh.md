...
宿主是权威的附件前置检查点。它会根据当前的单张字节数、图片数量、总字节数、媒体类型、尺寸和像素数策略校验整个批次，再写入附件或事件；拒绝会通过 composer 的短时 toast 显示。模型原生能力不参与准入：支持图片的路由接收持久图片块，明确排除图片输入的路由则在可用时接收包含当前工具可读规范化路径的确定性文本（[投影决策](../architecture/2026-09-17-tool-readable-images-and-video-audio-capability.zh.md)）。这使返回文本的工具能够检查已保存附件；没有这种工具或路径时，模型必须报告限制。`session.updateQueue` 只接受文本内容，因此队列编辑无法绕过附件准入注入图片。浏览器会在分配预览 URL 前拒绝声明不支持的图片媒体类型，但不会为部署限制或模型能力保留快照。
