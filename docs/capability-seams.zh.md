...
flowchart LR
  ...
  pkg_llm_files["llm-files"]
  svc_llmFiles["ctx.llmFiles<br/>Ephemeral provider file uploads"]
...
| `ctx.llmFiles` | `seam` | [`llm-files`](../packages/llm/llm-files) | - | - | - | 提供方插件注册上传、配额恢复和删除操作；模型适配器共享按摘要索引的上传结果，但不持久化提供方文件标识符。 |
