...
请求会对照其精确模型的能力——上下文窗口、输出默认值、推理强度、协议、输入模态、视频音轨模式与文件策略——校验，填入任何适配器配置的默认值，然后整个请求被深度冻结。`prepareCall()` 把这些事实、分离的上下文与重试策略绑定到执行最终分发的精确适配器代次，因此 HMR 或动态设置无法把一个代次的媒体能力与另一代次的端点混用。支持图片的适配器把持久图片引用投影为路由专用请求版本。明确不支持原生图片输入的路由会改为接收确定性文本；当附件与文件系统提供方能够暴露路径时，该文本包含当前工具可读取的规范化图片路径，使返回文本分析的工具能够检查文件，而不会声称模型已经看过图片。内置 `read_image` 工具仍要求原生图片输入，因为它的结果包含图片块。只有精确模型同时声明相应模态和原生文件策略时，带有已校验视频、音频或文档 MIME 类型的通用 `FileBlock` 记录才会变成提供方无关媒体块；其他文件继续使用指出只读保存路径的确定性 handle 文本。模型不接受的显式媒体块会变成确定性省略文本，且不会改写仅追加 Session 历史。`offloadRequestImagesWithPolicy()` 按原始字节或 base64 大小以及图片数或字节步长，确定性地从最旧图片开始移除；纯函数 `offloadedImagePrefixCount()` 公开同一决策，使路由所属的请求定价无需构建投影即可复现它。对视觉 token 收费的适配器声明按路由的 `imageRequestPricing`，`ctx.llm.imageRequestPricing(provider, model)` 为 token meter 同步解析它。分发经过 `llm/stream` waterfall，随后分片以 token 级增量返回，每个适配器结果都以唯一一个终止 `finish` 分片到达消费方。
...
- **媒体投影遵循精确能力**——只有模型同时声明模态和原生文件策略时，已校验视频、音频与文档文件才会到达适配器；其他文件保留确定性 handle 文本。
- **视频音轨是独立能力**——`videoAudioMode: 'visual-only'` 表示原生视频输入只覆盖画面，`'visual-and-audio'` 还保证理解内嵌音轨；独立 `audio` 仍是另一种输入模态。
