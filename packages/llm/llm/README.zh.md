...
| [`src/content.ts`](src/content.ts) | 共享图片与媒体投影辅助函数，包括请求图片卸载 |
| [`src/api-key.ts`](src/api-key.ts) | 每个适配器共享的凭据格式校验 |
| [`src/adapter-failure.ts`](src/adapter-failure.ts) | 把失败归一化为终止 finish 分片 |

### 主流程

请求会对照其精确模型的能力——上下文窗口、输出默认值、推理强度、协议、输入模态、视频音轨模式与文件策略——校验，填入任何适配器配置的默认值，然后整个请求被深度冻结。`prepareCall()` 把这些事实、分离的上下文与重试策略绑定到执行最终分发的精确适配器代次，因此 HMR 或动态设置无法把一个代次的媒体能力与另一代次的端点混用。支持图片的适配器把持久图片引用投影为路由专用请求版本。明确不支持原生图片输入的路由会改为接收确定性文本；当附件与文件系统提供方能够暴露路径时，该文本包含当前工具可读取的规范化图片路径，使返回文本分析的工具能够检查文件，而不会声称模型已经看过图片。内置 `read_image` 工具仍要求原生图片输入，因为它的结果包含图片块。只有精确模型同时声明相应模态和原生文件策略时，带有已校验视频、音频或文档 MIME 类型的通用 `FileBlock` 记录才会变成提供方无关媒体块。不受支持的通用文件与显式媒体会变成确定性的工具路径句柄；没有可读路径时，句柄要求模型报告限制。`visual-only` 视频路由保留原生视频块，并额外收到一个句柄，用于在需要时通过工具分析内嵌音轨；`visual-and-audio` 路由只接收视频。这些请求投影不会改写仅追加 Session 历史。`offloadRequestImagesWithPolicy()` 按原始字节或 base64 大小以及图片数或字节步长，确定性地从最旧图片开始移除；纯函数 `offloadedImagePrefixCount()` 公开同一决策，使路由所属的请求定价无需构建投影即可复现它。对视觉 token 收费的适配器声明按路由的 `imageRequestPricing`，`ctx.llm.imageRequestPricing(provider, model)` 为 token meter 同步解析它。分发经过 `llm/stream` waterfall，随后分片以 token 级增量返回，每个适配器结果都以唯一一个终止 `finish` 分片到达消费方。
...
- **图片投影遵循捕获的路由**——只有支持图片的模型会把持久 `ImageBlock` 引用转换为路由专用请求版本；当前执行文件系统能够读取规范化附件时，纯文本模型接收稳定的工具路径句柄，否则接收明确的无路径限制说明。
- **媒体投影遵循精确能力**——只有模型同时声明模态和原生文件策略时，已校验视频、音频与文档文件才会到达适配器；不受支持的媒体会得到当前工具可读路径或明确的无路径限制说明。
- **视频音轨是独立能力**——`videoAudioMode: 'visual-only'` 表示原生视频输入只覆盖画面，并为音轨附加工具路径句柄；`'visual-and-audio'` 还保证原生理解内嵌音轨；独立 `audio` 仍是另一种输入模态。
...
对于明确不支持原生图片输入的路由，服务会把每个持久 `ImageBlock` 替换为确定性说明，并在可用时附上当前只读执行路径。模型可以把该路径交给返回文本分析的工具；如果不存在这种工具或路径，说明会要求模型报告限制，而不能声称已经检查图片。原生图片路由继续接收图片块。

#### Token 影响

说明文本与可选路径会在纯文本路由上消耗输入 token。原生图片路由仍采用提供方特定的图片 token 计量方式。
...
视频、音频和文档附件遵循与图片相同的恢复规则。具备原生能力的路由接收媒体块，其他路由接收包含附件身份和当前只读执行路径的确定性文本，使可用工具能够返回文字分析。没有路径或合适工具时，模型必须报告限制，不能声称已经检查附件。只理解画面的视频路由保留视觉输入，并专门为内嵌音轨接收同一个恢复路径。

#### Token 影响

降级句柄消耗文本输入 token，而不是提供方的原生媒体 token。只理解画面的视频会消耗正常视觉 token，并额外消耗内嵌音轨句柄的文本 token。

#### KV Cache 影响

附件身份与能力说明是确定性的，执行环境路径则按请求解析。路径变化或原生投影与降级投影之间的切换可能从该附件起阻止前缀复用。
