...
你可以把图片和通用文件附加到提示词中，harness 会持久保存它们：每张源图都会在你的消息被处理前准入并规范化，其他文件则按字节原样保存，两者都会在同一会话重启后重新出现在对话历史中。随附的 `dsh` 组合无需任何配置即可支持这一点。浏览器路径、提供方 URL、本地存储路径与 base64 绝不会进入持久会话事件。存储会从文件字节校验可用的文档、音频和视频 MIME 类型；精确模型随后可以通过声明的原生文件策略接收持久引用，不支持的文件仍使用保存的只读路径回退。已存储对象永远不会被自动删除。
...
- **文件原样，图片规范化。**`saveFile` 提交已有字节数组，`saveFileStream` 以背压和取消语义提交有界分块，`readFileStream` 校验并返回有界分块，`fileHostPath` 定位存储对象供按需读取投影。两种文件写入路径都会在记录可用的文档、音频或视频 MIME 类型前检查字节；受支持的声明类型若与字节冲突会关闭式失败。`dsh-llm` 中的 `FileBlock` 内容块承载 `FileAttachmentRef`，请求组装会为有能力的路由把它提升为提供方无关媒体，否则保留确定性 handle 文本。
...
- **光栅图片限制只作用于图片**——图片接受部署限额内的 PNG、JPEG、WebP 与 GIF；通用文件逐字节存储，而原生媒体资格仅限 `src/media.ts` 检查的文档、音频和视频类型。
