...
你可以把图片和通用文件附加到提示词中，harness 会持久保存它们：每张源图都会在你的消息被处理前准入并规范化，其他文件则按字节原样保存，两者都会在同一会话重启后重新出现在对话历史中。随附的 `dsh` 组合无需任何配置即可支持这一点。浏览器路径、提供方 URL、本地存储路径与 base64 绝不会进入持久会话事件。存储会从文件字节校验可用的光栅图片、文档、音频和视频 MIME 类型；精确模型随后可以通过声明的原生文件策略接收持久引用，不支持的文件仍使用保存的只读路径回退。已存储对象永远不会被自动删除。
...
任何非图片文件都以通用文件的形式附加到提示词：确切字节被只读保存在 harness 主目录下，消息记录清洗后的名称、字节数、内容摘要，以及符合原生模型输入条件时的已校验 MIME 类型。只有精确路由声明对应模态和文件策略时，模型才会收到原生视频、音频或文档内容；否则仍收到一行保存路径。本服务的存储保持逐字节原样，且自身不设大小上限。
...
- **文件原样，图片规范化。**`saveFile` 提交已有字节数组，`saveFileStream` 以背压和取消语义提交有界分块，`readFileStream` 校验并返回有界分块，`fileHostPath` 定位存储对象供按需读取投影。两种文件写入路径都会在记录可用的光栅图片、文档、音频或视频 MIME 类型前检查字节；受支持的声明类型若与字节冲突会关闭式失败。`dsh-llm` 中的 `FileBlock` 内容块承载 `FileAttachmentRef`，请求组装会为有能力的路由把它提升为提供方无关媒体，否则保留确定性 handle 文本。
...
该包通过提供方适配器间接影响模型；适配器会按精确模型声明的能力解析持久图片和媒体引用。受支持的视频、音频或文档可以成为提供方原生输入；不支持的通用文件仍是一行确定性 handle 文本，包含字节数、摘要前缀和保存的只读路径。
...
- **光栅图片限制只作用于图片**——图片接受部署限额内的 PNG、JPEG、WebP 与 GIF；通用文件逐字节存储，而原生媒体资格仅限 `src/media.ts` 检查的光栅图片、文档、音频和视频签名。
...
当前生产适配器声明只输出文本，因此只有用户内容携带图片、视频、音频和文档。助手侧媒体输出仍未决定。
