...
此 Cordis 客户端插件将组织账户、授权模型、用量、设备和已发布插件视图加入现有 DSH Web 设置壳。它保持 [Web 用户端](../../../apps/web/src/main.ts) 的用户界面布局与交互，不渲染管理后台。
...
此插件是 profile 组件，不是第二套聊天应用。Web profile 负责聊天、会话、工具、工作区、附件、计划、目标、任务、搜索、导出和定时任务视图。企业策略会在加载此插件前禁用本地模型和个人插件设置。企业“模型”页面只列出服务端授权目录，并通过本地 Host 写入 `enterprise` 默认模型；网关会在每次调用时再次校验授权。
...
Host 测试验证凭据隔离和 fail-closed 授权。jsdom 套件验证设置槽注册、中文渲染、授权模型与用量显示、设备二次确认以及已发布目录字段。
