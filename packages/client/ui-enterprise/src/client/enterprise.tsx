...
function AccountSection({ loadDashboard, revokeRuntime, switchOrganization, logout, t }: AccountProps): ReactNode {
  ...
  return <div className="dse-section">
    ...
    <div className="dse-grid">
      ...
      <section className="dse-card"><h3>{t('budget')}</h3><p className="dse-value">{money(data.subscription.spentMicros)}</p><p className="dse-meta">{money(data.subscription.reservedMicros)} reserved · {money(data.subscription.budgetMicros)} limit</p></section>
